...
*/
void setmodemparams(int baudrate) /* setup serial port */
{
...
*/  
int read_from_modem(char *buffer,int no_of_bytes,int timeout)
{
  struct sigaction oldact,newact;
  int count=0,tot_bytes_read=0; 
...
/*
  ...
  Returns : 0 on Success
          :-1 if it fails.
*/
int write_to_modem(char *buffer,int no_of_bytes)
{
...
*/
int Exec_Modem_command(char *command,char *response,int timeout)
{
  ...
  do
      ...
      if(count>0)
	{
	  ...
	  if(ch==0x0a&&response[tot_bytes_read-1]==0x0d){modemflag++;}
	  tot_bytes_read+=count;
...
/*------------------------Timeout Handler-----------------------------------------------------------*/
void modem_timeout_handler(int signum,siginfo_t* sinfo,void *ctxt)
{
...
*/ 
int exec_modem_comm_without_signal(char *command,char *response,int timeout)
{
  struct timeval tv;
  int result,count=0,tot_bytes_read=0,bytes_avlbl=0,index=0;
  ...
  do
      ...
      if(result>0)
	{
	  ...
	  for(;((index<strlen(response))&&(strlen(response)>1));index++)
            if(response[index]==0x0a&&response[index-1]==0x0d)cntr++;
	}
...
*/ 
int Test_modem_response_without_signal(char *command,char *response,char *expect[],int timeout)
{
  struct timeval tv;
  int result,count=0,tot_bytes_read=0,bytes_avlbl=0;
...
*/  
int read_from_modem_without_signal(char *buffer,int no_of_bytes,int timeout)
{
  struct timeval tv;
  int result,count=0,tot_bytes_read=0,bytes_avlbl=0;
